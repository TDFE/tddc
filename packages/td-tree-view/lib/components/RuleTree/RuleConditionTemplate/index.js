...
var InputContext = {
  input: _otp.default.changliang,
  context: _otp.default.bianliang
};
var RuleConditionTemplate = function RuleConditionTemplate(props) {
  ...
  var cfgJson = currentTemplate && currentTemplate.cfgJson && (0, _utils.isJSON)(currentTemplate.cfgJson) ? JSON.parse(currentTemplate.cfgJson) : null;
  ...
  var getSelfSelectName = function getSelfSelectName(options, value, mode) {
    ...
    // 单选
    var selfObj = options && options.length && options.find(function (v) {
  ...
  var getServiceSelectName = function getServiceSelectName(options, value, serviceMapName, mode) {
    ...
    // 单选
    var serviceObj = options && options.length && options.find(function (v) {
      return v.name === value;
    }) || {};
    if (serviceMapName === 'ruleAndIndexFieldList') {
      return "[".concat(serviceObj.sourceName && sourceName(serviceObj.sourceName, lang), "] ").concat(lang === 'en' ? serviceObj.name : serviceObj.dName);
    }
  ...
  var getCheckboxName = function getCheckboxName(options, value) {
    ...
    options && options.length && options.map(function (option) {
      var _value;
      if ((_value = value) === null || _value === void 0 ? void 0 : _value.includes(option.value)) {
        optionNames.push(lang === 'en' ? option.enName : option.name);
      }
    });
    return optionNames.join(lang === 'en' ? ',' : '，');
  ...
  var getContextRightName = function getContextRightName(value, includeIndex) {
    ...
    var optionsObj = options && options.length ? options.find(function (v) {
      return v.name === value;
    }) || {} : {};
    if (includeIndex) {
      return "[".concat(sourceName(optionsObj.sourceName, lang), "] ").concat(lang === 'en' ? optionsObj.name : optionsObj.dName);
    }
  ...
  property && _constants.excludeRuleTemplate.includes(property) && /*#__PURE__*/_react.default.createElement(_CustomRuleConfig.default, {
    data: data,
    simpleCfgList: simpleCfgList,
    lang: lang
  }), property && !_constants.excludeRuleTemplate.includes(property) && (cfgJson === null || cfgJson === void 0 ? void 0 : cfgJson.params) && (cfgJson === null || cfgJson === void 0 ? void 0 : cfgJson.params.map(function (item, index) {
    return /*#__PURE__*/_react.default.createElement("span", {
      className: "rule-condition-template",
      key: index
    }, lang === 'en' ? item.enLabelText : item.labelText, " \uFF1A", item.children && item.children.map(function (subItem) {
      var _ref3 = subItem || {},
        name = _ref3.name,
      ...
      /*
                    ...
                    * */
      var currentSimpleObj = simpleCfgList.find(function (fItem) {
      ...
      // 获取handle 实体
      var changeHandleObj = changeHandleName ? simpleCfgList.find(function (fItem) {
      ...
      if (changeMode === 'whenSomeValue') {
        // 当为具体值的时候
        caseList && caseList.map(function (caseItem) {
          if (caseItem['modeValueList'] && caseItem['modeValueList'].find(function (mvItem) {
      ...
      return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, componentType === 'input' && "".concat(valueByScene).concat(addonAfter || ''), componentType === 'select' && selectType === 'self' && (getSelfSelectName(selectOption, valueByScene, mode) || valueByScene), componentType === 'select' && selectType === 'service' && (getServiceSelectName(allMap[serviceMapName], valueByScene, serviceMapName, mode) || valueByScene), componentType === 'checkbox' && selectType === 'self' && (getCheckboxName(selectOption, value) || value), componentType === 'checkbox' && selectType === 'service' && (getCheckboxName(allMap[subItem.selectName], value) || value), componentType === 'variable' && /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, InputContext[type], type === 'input' && (value || ''), type === 'context' && getContextRightName(value, includeIndex)));
    }));
  })));
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, currentTemplate && currentTemplate.description && /*#__PURE__*/_react.default.createElement(_tooltip.default, {
    title: dom,
    overlayClassName: "template-node"
  }, /*#__PURE__*/_react.default.createElement("span", {
    className: "template-des"
  }, /*#__PURE__*/_react.default.createElement(_tntd.Ellipsis, {
    title: "".concat(_otp.default.guize).concat(description),
    placement: "bottom",
    widthLimit: 450
  }))));
};
var _default = exports.default = RuleConditionTemplate;