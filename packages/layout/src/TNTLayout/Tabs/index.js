...
export const HeaderTabs = WrapLocaleReceiver((props) => {
  const {
    className,
    tabs = [],
    onTabChange,
    ...
    <div className={cn('tnt-layout-header-tabs', className)}>
      ...
      <div className="right-menulist" style={{ ...style }}>
        <div onClick={onCloseNow} className={`${tabs.length === 1 ? 'disabled' : ''}`}>
          {I18N.tabs.index.guanBi}
        </div>
        <div onClick={onCloseOthers}>{I18N.tabs.index.guanBiQiTa}</div>
        <div onClick={onCloseRight}>{I18N.tabs.index.guanBiYouCe}</div>
      </div>
      {arrowVisible && (
