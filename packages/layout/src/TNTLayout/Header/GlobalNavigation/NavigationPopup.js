...
export default WrapLocaleReceiver(
  ({
    menus = [],
    close,
    popupRef,
    ...
    return (
      <>
        <div className={cn('search-area', { active })}>
          <Icon type="search" />
          <input
            placeholder={I18N.globalnavigation.navigationpopup.caiDanSouSuo}
            value={searchValue}
          ...
          {searchedMenus?.length > 0 && (
            <SearchList items={searchedMenus} onSelectMenu={onMenuClick} />
          )}
        </div>
        {isHasLevel3 && (
