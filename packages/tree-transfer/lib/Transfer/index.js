...
var TreeTransfer = function TreeTransfer(_ref) {
  ...
  var changeDataSource = (0, _react.useCallback)(function () {
    ...
    setLeftTree(_objectSpread(_objectSpread({}, leftTree), {}, {
      dataSource: newLeftTreeDataSource,
      expandedKeys: defaultExpandAll ? (0, _utils.getLastLevelData)(newDataSource).map(function (v) {
        return v.key;
      }) : []
    }));
    setRightTree(_objectSpread(_objectSpread({}, rightTree), {}, {
  ...
  var changeState = function changeState(direction, data, newLeftTree, newRightTree, callback) {
    ...
    direction === 'left' ? setLeftTree(_objectSpread(_objectSpread({}, leftTreeV), data)) : setRightTree(_objectSpread(_objectSpread({}, rightTreeV), data));
    callback && callback({
      selectValues: selectValues,
      leftTree: direction === 'left' ? _objectSpread(_objectSpread({}, leftTreeV), data) : leftTreeV,
      rightTree: direction === 'left' ? rightTreeV : _objectSpread(_objectSpread({}, rightTreeV), data)
    });
  };
  var operationOnCheck = function operationOnCheck(keys, data, direction, rightToLeft, leftTree, rightTree, callback) {
  ...
  var _onCheck = function onCheck(keys, info, direction, rightToLeft, callback) {
    ...
    if (direction === 'left') {
      var newLeftTree = _objectSpread(_objectSpread({}, leftTree), {}, {
        ...
        keys: rightToLeft ? _lodash.default.uniq([].concat(_toConsumableArray(selectValues), _toConsumableArray(leftTree.checkedKeys))) : _lodash.default.uniq([].concat(_toConsumableArray(selectValues), _toConsumableArray(lastLevelKey)))
      });
      setLeftTree(newLeftTree);
      var newKeys = _lodash.default.uniq([].concat(_toConsumableArray(lastLevelKey), _toConsumableArray(selectValues)));
      operationOnCheck(newKeys, dataSource, direction, rightToLeft, newLeftTree, rightTree, callback);
    } else {
      // 选择的是右侧的Tree时只需要改变受控的keys然后调用operationOnCheck方法
      var newRightTree = _objectSpread(_objectSpread({}, rightTree), {}, {
        checkedKeys: lastLevelKey,
        keys: lastLevelKey
      });
      setRightTree(newRightTree);
      operationOnCheck(lastLevelKey, newRightTree.dataSource, direction, rightToLeft, leftTree, newRightTree);
    }
  };
  ...
  var leftToRight = function leftToRight() {
    ...
    if (!_lodash.default.isEmpty(newRightTree.checkedKeys)) {
      operationOnCheck(newRightTree.checkedKeys, newRightTree.dataSource, 'right', false, newLeftTree, newRightTree);
    }
    // 返回给父组件数据
  ...
  var renderCheckBox = function renderCheckBox(direction) {
    ...
    var checkAllDisabled = disabled || directionDisabled || _lodash.default.isEmpty(operationState.dataSource); // 全选的checkbox是否disabled
    // 全选或者全不选的状态
  ...
  var checkAll = function checkAll(direction, type) {
    ...
    var operationState = direction === 'left' ? leftTree : rightTree;
    var selectAllKeys = (0, _utils.getLastLevelData)(operationState.dataSource).map(function (item) {
      return item.key;
    });
    // 全选右侧时所有的key
    ...
    changeState(direction, {
      selectDataSource: directionDisabled ? (0, _utils.disabledCategoryData)(dataSource) : dataSource,
      filterSelectDataSource: [],
  ...
  }, renderCheckBox('left')), showSearch && /*#__PURE__*/_react.default.createElement("div", {
  ...
  , {
    ...
    placeholder: _lodash.default.get(searchPlaceholder, 0, _locale.default.treetransfer.index.qingShuRu)
  }))), _lodash.default.isEmpty(leftTree.dataSource) ? /*#__PURE__*/_react.default.createElement("div", {
    className: "td-tree-transfer-no-data"
  }, notFoundContent) : /*#__PURE__*/_react.default.createElement("div", {
    ...
    className: "td-tree-transfer-box-select-all"
  }, renderCheckBox('right')), showSearch && /*#__PURE__*/_react.default.createElement("div", {
    className: "td-tree-transfer-box-search"
  }, /*#__PURE__*/_react.default.createElement(Search
  // style={{ width: "95%", marginBottom: "10px" }}
  , {
    ...
    placeholder: _lodash.default.get(searchPlaceholder, 1, _locale.default.treetransfer.index.qingShuRu)
  }))), _lodash.default.isEmpty(rightTree.dataSource) ? /*#__PURE__*/_react.default.createElement("div", {
    className: "td-tree-transfer-no-data"
  }, notFoundContent) : /*#__PURE__*/_react.default.createElement("div", {
    className: "td-tree-transfer-tree"
  }, /*#__PURE__*/_react.default.createElement(_tree.default, {
...
TreeTransfer.defaultProps = {
  ...
  searchPlaceholder: [_locale.default.treetransfer.index.qingShuRu, _locale.default.treetransfer.index.qingShuRu],
